...
def build_model(model_type, weights, layerToEngage, PAV):
    model = None
    ...
    # model.add(Dense(512))
    MakePatchHiddenLayers(model, PAV)
...
def buildModelName(opt_type, model_type, drift_type, layer, PAV=None): 
    if PAV is None:
        return "models/mnist_{0}_{1}_{2}_{3}.h5".format(opt_type, model_type, drift_type, layer)
    else:
        return "models/mnist_{0}_{1}_{2}_{3}_{4}.h5".format(opt_type, model_type, drift_type, layer, PAV)
...
def buildOrLoadModel(h5FileName, model_type, layerToEngage, PVA):
    ...
    if h5file.is_file():
        ...
        return build_model(model_type, "", layerToEngage, PVA)
