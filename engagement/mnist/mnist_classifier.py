...
def calc_accuracy_using_Ei(modelC0, modelEi, modelCi, X, y):
...
def build_model(model_type, weights):
    ...
    if model_type == "E":
        ...
        model.add(Dense(2, name="MS"))
        model.add(Activation('softmax', name="MS_Act"))
...
model_ms = build_model("P", C0Weights)
...
for i in range(nbBaseBatches, nbBatches):
    ...
    accMSPi.append(calc_accuracy(modle_C0, model_ms, model_P, X, y))
    x_Ei = []
    y_Ei = []
    x_ms = []
    y_ms = []
    ...
    for index in range(len(X)):
        ...
        predictP = model_P.predict(X[index].reshape(1, 28, 28, 2), batch_size=1)
    ...
    accArray_Base_Updated.append(np.mean(h_base_updated["categorical_accuracy"]))

    h_freezing = model_freezing.fit(X, y, batch_size=50, epochs=10)
    accArray_Freezing.append(np.mean(h_freezing["categorical_accuracy"]))
