...
for i in range(nbBaseBatches, nbBatches):
    ...
    if len(x_Pi) > 0:
        ...
        y_Pi = to_categorical(y_Pi, 10)
        loss_P, acc_P = model_P.evaluate(x_Pi, y_Pi, batch_size=50)
        accArray_P.append(acc_P)
        h_Pi = model_P.fit(x_Pi, y_Pi, batch_size = 50, epochs = 10)
        accArray_P.append(np.mean(h_Pi.history["categorical_accuracy"]))
        
        loss_bu, acc_bu = model_Base_Updated.evaluate(x_Pi, y_Pi, batch_size=50)
        accArray_Base_Updated.append(acc_bu)
        h_base_updated = model_Base_Updated.fit(x_Pi, y_Pi, batch_size=50, epochs=10)

        loss_freezing, acc_freezing = model_freezing.evaluate(x_Pi, y_Pi, batch_size=50)
        accArray_Freezing.append(acc_freezing)
        h_freezing = model_freezing.fit(x_Pi, y_Pi, batch_size=50, epochs=10)
        
    else:
        accArray_P.append(None)
        accArray_Base_Updated.append(None)
        accArray_Freezing.append(None)
